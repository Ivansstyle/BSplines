 ...
 */
Canvas::Canvas() : m_currentOperation(Operation::CURVE), m_currentColour(Operation::RED)
{
...
void Canvas::draw(SDL_Renderer* r) {
    ...
    {
      SDL_RenderDrawPoint(r,coords[i],coords[i+1]);
      SDL_RenderDrawPoint(r,coords[i]+1,coords[i+1]);
      SDL_RenderDrawPoint(r,coords[i]+1,coords[i+1]+1);
      SDL_RenderDrawPoint(r,coords[i],coords[i+1]+1);
    }

    for(int i=0; i<(int)coords.size(); i+=2)
    {
      SDL_RenderDrawLine(r, coords[i], coords[i+1], coords[i+2], coords[i+3]);
    }
...
void Canvas::mouseDown(int x, int y) {
    ...
    case (Operation::RECTANGLE):
        ...
        break;
    case (Operation::CURVE):

      if(curvestate==0)
      {
        std::cout<<""<<std::endl;
        op = new CurveOperation(x,y,x,y);
        coords.push_front((double)y);
        coords.push_front((double)x);
      ...
      else
      {
        ...
        op = new CurveOperation(coords[0],coords[1],coords[6],coords[7]);
        op->setFirstPoint(coords[2],coords[3]);
        op->setSecondPoint(coords[4],coords[5]);
...
void Canvas::mouseMove(int x, int y) {
  /*
  if (m_mouseDown && !m_curveinmotion) {
